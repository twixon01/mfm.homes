...
function normalizeEbayImageKey(url: string) {
  const withoutQuery = url.split("?")[0];
  return withoutQuery.replace(/(.*)\/s-l\d+(\.[a-z0-9]+)$/i, "$1$2");
}

function toHighResEbayImageUrl(url: string) {
  const withoutQuery = url.split("?")[0];
  return withoutQuery.replace(/\/s-l\d+(?=\.[a-z0-9]+$)/i, "/s-l1600");
}

function extractImageUrls(values: Array<string | undefined | null>) {
  const byKey = new Map<string, string>();
  for (const rawValue of values) {
    const trimmed = rawValue?.trim();
    if (!trimmed) continue;
    const highRes = toHighResEbayImageUrl(trimmed);
    const key = normalizeEbayImageKey(highRes);
    if (!byKey.has(key)) {
      byKey.set(key, highRes);
    }
  }
...
async function getEbayItemDetails(token: string, itemId: string) {
  try {
    ...
    return {
      ...
      images: extractImageUrls([payload.image?.imageUrl, ...(payload.additionalImages?.map((image) => image.imageUrl) ?? [])]),
  ...
  try {
    ...
    const ebayMapped = await Promise.all(
      items.map(async (item) => {
        ...
        const summaryImage = item.image?.imageUrl;
        ...
        const images = extractImageUrls([...(details?.images ?? []), summaryImage]);
        const sizeAspects =
